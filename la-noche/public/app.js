...
async function home(){
  ...
  const refreshExtras=()=>{const when=document.querySelector('input[name="when"]:checked').value;document.querySelector("#dateWrap").classList.toggle("show",when==="later");const theme=document.querySelector('input[name="theme"]:checked').value;document.querySelector("#ageWrap").classList.toggle("show",theme==="picante18")};
...
function paywall(r){
  app.innerHTML=brand()+`<section class="card center paywall"><div class="big-num">🔥</div><div class="section-title">La primera ronda terminó.</div><p class="lead" style="margin-left:auto;margin-right:auto">Los puntos y la respuesta quedaron guardados. Tu grupo generó <strong>${Math.max(0,r.totalRounds-1)} rondas más</strong>.</p><div class="paywall-grid">${r.availableModes.filter(m=>["quien_fue","lee_al_grupo","mentiroso","silla_caliente","todos_contra_uno"].includes(m.id)).map(m=>`<div><span>${m.emoji}</span><strong>${esc(m.title)}</strong></div>`).join("")}</div><div class="divider"></div>${r.isHost?'<button class="primary wide" id="unlockTest">Desbloquear en modo prueba →</button><div class="tiny muted" style="margin-top:10px">Después este botón será el pago real.</div>':'<div class="muted">Esperando que el host desbloquee la noche…</div>'}</section>${hostRoster(r)}`;
  if(r.isHost)document.querySelector("#unlockTest").onclick=async()=>{try{await api("/api/rooms/"+r.code+"/unlock-test",{method:"POST"});refresh()}catch(e){toast(e.message)}};
  bindHostRoster(r);
}
