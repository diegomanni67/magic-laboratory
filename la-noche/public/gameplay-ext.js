function playing(r){
  ...
  app.innerHTML=brand()+'<div class="statusbar"><div><div class="kicker mode-kicker">'+assetImg("mode",x.mode,"mode-asset")+esc(x.modeTitle)+'</div><div class="tiny muted">'+esc(r.theme.title)+' · Ronda '+(r.currentRound+1)+'/'+r.totalRounds+'</div></div><span class="pill">'+x.voteCount+'/'+x.eligibleVoters+' votos</span></div><section class="card prompt-card"><div class="tiny muted">'+esc(x.prompt)+'</div><div class="statement">“'+esc(x.statement)+'”</div>'+interaction+'</section>'+mission+'<section class="card soft hidden-score" style="margin-top:14px"><div class="kicker">🏆 PUNTAJE SELLADO</div><div class="section-title">Nadie sabe quién va ganando.</div><div class="muted">Dúos, rankings, misiones y aciertos se suman automáticamente. Todo se abre al final.</div></section>'+hostRoster(r);
